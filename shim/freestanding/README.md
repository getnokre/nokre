...
what is left is seven declarations and the four definitions behind them
(`freestanding.c`). Everything here exists to keep nokre's one vendored
C file compiling where there is no C library, which is what the DOM
edition's live driver targets (`docs/internals/dom-edition.md`): no
Skia, no emscripten, no libc.
