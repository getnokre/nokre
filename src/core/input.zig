//! Pointer, keyboard, and scroll interaction: hit testing, activation,
//! focus reveal, the scroll chain, and the one gesture — the edge pan
//! that goes back. Everything arrives through `App.dispatch`; text-field
//! editing is delegated to editing.zig, the modal layers to overlays.zig.
...
/// rects the renderer underlined — `layout.spanRects` is the one walk
...
fn visibleRect(app: *App, id: NodeId) geometry.Rect {
...
pub fn handleKey(app: *App, key: event_mod.Key, mods: event_mod.Modifiers) !void {
    ...
    switch (el.*) {
        ...
        .segmented => |*s| switch (key) {
            .left, .right => {
                ...
                if (forward) {
                    if (s.selected + 1 < s.options.len) {
                        s.selected += 1;
                        revealSegSelected(app, focused_id);
            ...
            else => handleRootScrollKey(app, key),
        },
        ...
        .code_block => switch (key) {
            .left, .right => {
                ...
                scrollHorizontallyBy(app, focused_id, if (key == .right) step else -step);
            },
            else => handleRootScrollKey(app, key),
        },
        ...
        .scroll_region => |sr| {
            ...
            switch (key) {
                .up => _ = scrollRegionBy(app, focused_id, -line),
                .down => _ = scrollRegionBy(app, focused_id, line),
                .page_up => _ = scrollRegionBy(app, focused_id, -(h - line)),
                .page_down => _ = scrollRegionBy(app, focused_id, h - line),
                .home => _ = scrollRegionBy(app, focused_id, -sr.content_height),
                .end => _ = scrollRegionBy(app, focused_id, sr.content_height),
                else => {},
            }
        },
        else => switch (key) {
...
fn innermostRegionAt(app: *App, at: Point) ?NodeId {
    ...
    while (it.next()) |id| {
        if (app.tree.getConst(id).?.role() != .scroll_region) continue;
        if (!visibleRect(app, id).contains(at)) continue;
        found = id;
    }
    return found;
}
...
fn scrollHorizontallyBy(app: *App, id: NodeId, delta: i32) void {
    const el = app.tree.get(id) orelse return;
    switch (el.*) {
        .segmented => |*s| {
...
fn revealSegSelected(app: *App, id: NodeId) void {
    const el = app.tree.get(id) orelse return;
    const s = &el.segmented;
    const inner_w = layout.segTrackWindow(app.tree.rectOf(id), s.bleed).w;
...
fn scrollRegionBy(app: *App, id: NodeId, delta: i32) i32 {
...
fn scrollRootBy(app: *App, delta: i32) void {
...
pub fn revealFocused(app: *App) void {
    ...
    while (parent) |pid| : (parent = app.tree.parentOf(pid)) {
        ...
        if (delta != 0) {
            _ = scrollRegionBy(app, pid, delta);
            app.performLayout();
        }
    }
    ...
    if (delta != 0) scrollRootBy(app, delta);
}
...
/// above. All three read `layout.spanRects`, so they cannot disagree.
pub fn spanRectsOf(app: *App, id: NodeId, span_index: u16, out: []geometry.Rect) []geometry.Rect {
    ...
    return layout.spanRects(app.measurer, app.bidi_scratch, run.face, run.scale, run.content, run.spans, span_index, app.tree.rectOf(id), out);
}
