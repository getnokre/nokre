...
pub fn tileGroupDescHeight(measurer: text.Measurer, description: []const u8, avail_w: i32) i32 {
    ...
    var lines: i32 = 0;
    var it = wrap(measurer, .prose, text.Scale.small.px(), description, avail_w);
...
const Ctx = struct {
        ...
        return switch (el.*) {
            .text, .heading => blk: {
                ...
                const w = if (run.spans.len == 0)
                    ...
                    spanTextWidth(self.measurer, run.face, run.scale.px(), run.spans);
    ...
    fn wrappedHeight(self: *Ctx, base: text.Face, scale: text.Scale, content: []const u8, spans: []const element_mod.Span, max_w: i32) i32 {
        var lines: i32 = 0;
        var it = wrapSpans(self.measurer, base, scale.px(), content, spans, max_w);
        while (it.next()) |_| lines += 1;
